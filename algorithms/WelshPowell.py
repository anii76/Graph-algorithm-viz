...
def ColorGluton(G):
    ...
    c = [] ; cpt = 0 # each 10 iterations update file
    ...
    while ColoredVertices < len(D):
        ...
        # Update color values at each 10 iterations
        if (cpt % 2 == 0):
            with open(logfile, "a") as file:
                try:
    ...
    print(max(c))
    with open(logfile, "a") as file:
        c = [max(c)+1 if i == 0 else i for i in c ]
        file.write(str(c)+"\n")
        file.write(str(c)+"\n")
        file.write(str(c)+"\n")
...
def main_welsh(benchmark: str, results: list):
