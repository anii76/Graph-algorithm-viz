...
def genetic(G):
    ...
    while count < nbr_iter and stagnation < max_stagnation :
        ...
        if (count % 5 == 0):
            population_fitness = [fitness(S) for S in updated]
            mi = np.argmin(population_fitness)
            with open(logfile, "a") as file:
                file.write(str(updated[mi])+"\n")
        count+=1
        population = updated
...
def main_genetic(benchmark: str, results: list):
